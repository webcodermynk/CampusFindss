...
export default function FoundItemsManagement({ toast }) {
  ...
  const fetchItems = async () => {
    setLoading(true)
    try { const r = await fetch('/api/found-items'); setItems(await r.json()) }
          ...
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.86rem' }}>
              ...
              <tbody>
                ...
                : filtered.map((item,i) => (
                  <tr key={item._id} style={{ borderBottom:i<filtered.length-1?'1px solid var(--border,#e2e8f0)':'none', transition:'background .15s' }}
                    ...
                    <td style={{ padding:'0.85rem 1rem' }}><Badge status={item.status||'found'}/></td>
