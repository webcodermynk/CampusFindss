...
const Stat = ({ icon, label, value, color, bg, delta }) => (
  <div style={{ background:'var(--surface,#fff)', borderRadius:14, padding:'1.3rem', border:'1.5px solid var(--border,#e2e8f0)', position:'relative', overflow:'hidden' }}>
    ...
    {delta !== undefined && <div style={{ fontSize:'0.75rem', color: delta >= 0 ? '#10b981' : '#ef4444', fontWeight:700, marginTop:3 }}>{delta >= 0 ? '▲' : '▼'} {Math.abs(delta)}% vs last month</div>}
  </div>
)
...
export default function Analytics({ toast }) {
  ...
  const totalItems   = (summary.totalLost||0) + (summary.totalFound||0)
  const recoveryRate = totalItems > 0 ? Math.round(((summary.approvedClaims||0) / totalItems) * 100) : 0

  // Radar data for category comparison
  const radarData = categoryData.slice(0,6).map(c => ({
    subject: c.name, value: c.value
  }))

  // Bar data: monthly items with trend
  const trendData = monthlyData.map(m => ({
    ...m,
    total: (m.lost||0) + (m.found||0)
  }))
    ...
    <div style={{ background:'var(--bg,#f8fafc)', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'1.5rem 1.5rem 4rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:'1rem', marginBottom:'2rem' }}>
          ...
          <Stat icon="🔴" label="Lost Reports"    value={summary.totalLost||0}    color="#ef4444" bg="#fee2e2"/>
          <Stat icon="🟢" label="Found Items"     value={summary.totalFound||0}   color="#10b981" bg="#d1fae5"/>
          <Stat icon="📋" label="Total Claims"    value={summary.totalClaims||0}  color="#f59e0b" bg="#fef3c7"/>
          <Stat icon="✅" label="Approved Claims" value={summary.approvedClaims||0} color="#10b981" bg="#d1fae5"/>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'1.2rem' }}>
          ...
          {/* Monthly trend line */}
          <Card title="📅 Monthly Trend" sub="Items reported per month" span={2}>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={monthlyData} margin={{ top:5, right:20, left:-10, bottom:0 }}>
                ...
                <YAxis tick={{ fontSize:12, fill:'var(--text-3,#94a3b8)' }} axisLine={false} tickLine={false}/>
          ...
          {/* Weekly bar */}
          <Card title="📆 Weekly Activity" sub="Items reported each week">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={weeklyData} margin={{ top:5, right:10, left:-15, bottom:0 }}>
                ...
                <YAxis tick={{ fontSize:11, fill:'var(--text-3,#94a3b8)' }} axisLine={false} tickLine={false}/>
                <Tooltip contentStyle={tip}/>
          ...
          <Card title="📋 Claim Status" sub="Distribution of all claims">
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={claimStatusData.length > 0 ? claimStatusData : [{name:'No data',value:1}]} cx="50%" cy="50%" outerRadius={80} innerRadius={40} dataKey="value" label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>
                  {(claimStatusData.length > 0 ? claimStatusData : [{name:'No data',value:1}]).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]}/>)}
                </Pie>
                <Tooltip contentStyle={tip}/>
                <Legend wrapperStyle={{ fontSize:'0.78rem' }}/>
              </PieChart>
          ...
          <Card title="📂 Items by Category" sub="Most reported categories" span={2}>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={categoryData.slice(0,10)} layout="vertical" margin={{ top:0, right:20, left:10, bottom:0 }}>
                ...
                <XAxis type="number" tick={{ fontSize:11, fill:'var(--text-3,#94a3b8)' }} axisLine={false} tickLine={false}/>
