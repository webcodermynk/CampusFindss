...
const SettingCard = ({ icon, title, desc, children, badge }) => (
  <div style={{ background: "var(--card-bg,#fff)", borderRadius: 14, border: "1.5px solid var(--border-color,#dee2e6)", padding: "1.8rem", marginBottom: "1.2rem", boxShadow: "0 4px 15px rgba(0,0,0,0.04)" }}>
    <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem", marginBottom: "1.2rem" }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.3rem" }}>
          <h5 style={{ fontWeight: 700, color: "var(--text-color,#2c3e50)", margin: 0, fontSize: "1rem" }}>{title}</h5>
        ...
        <p style={{ color: "var(--text-muted,#6c757d)", fontSize: "0.85rem", margin: 0 }}>{desc}</p>
      </div>
    </div>
    {children}
  </div>
...
const Toggle = ({ value, onChange, label }) => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.6rem 0" }}>
    <span style={{ fontSize: "0.9rem", color: "var(--text-color,#2c3e50)", fontWeight: 500 }}>{label}</span>
    <button onClick={() => onChange(!value)} style={{
      width: 44, height: 24, borderRadius: 12, border: "none", cursor: "pointer", position: "relative",
      background: value ? "linear-gradient(135deg,#6366f1,#8b5cf6)" : "var(--border-color,#dee2e6)",
  ...
  return (
    <div style={{ background: "var(--bg-color,#f8f9fa)", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "1.5rem" }}>
        ...
        <SettingCard icon="🗑️" title="Data Retention" desc="Automatically clean up old, unresolved items to keep the database lean.">
          ...
          {autoDelete && (
            <div style={{ marginTop: "1rem" }}>
              <label style={{ fontWeight: 600, fontSize: "0.85rem", color: "var(--text-muted,#6c757d)", display: "block", marginBottom: "0.4rem" }}>Delete items older than:</label>
              <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
                <input type="range" min={30} max={365} step={30} value={deleteDays} onChange={e => setDeleteDays(+e.target.value)}
                  style={{ flex: 1, accentColor: "#6366f1" }} />
                <span style={{ fontWeight: 700, color: "var(--text-color,#2c3e50)", minWidth: 70, fontSize: "0.9rem" }}>{deleteDays} days</span>
              </div>
            </div>
          )}
        </SettingCard>
        ...
        {/* Upcoming features */}
        <h3 style={{ fontWeight: 800, color: "var(--text-color,#2c3e50)", marginBottom: "1rem", fontSize: "1.1rem" }}>🚀 Coming Soon</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: "1rem" }}>
          {UPCOMING.map(({ icon, title, desc, color }) => (
            <div key={title} style={{ background: "var(--card-bg,#fff)", borderRadius: 12, padding: "1.2rem", border: "1.5px solid var(--border-color,#dee2e6)", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: color }} />
              <div style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>{icon}</div>
              <div style={{ fontWeight: 700, color: "var(--text-color,#2c3e50)", fontSize: "0.9rem", marginBottom: "0.3rem" }}>{title}</div>
              <div style={{ fontSize: "0.8rem", color: "var(--text-muted,#6c757d)", lineHeight: 1.5 }}>{desc}</div>
