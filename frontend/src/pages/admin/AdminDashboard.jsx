...
const StatCard = ({ icon, label, value, color, bg, onClick, active }) => {
  ...
  return (
    <div onClick={onClick} style={{ background:"var(--card-bg,#fff)", borderRadius:16, padding:"1.4rem 1.3rem", border:"1.5px solid var(--border-color,#e8ecf0)", cursor:"pointer", position:"relative", overflow:"hidden", transition:"all 0.25s ease" }}
      ...
      <div style={{ fontSize:"2.1rem", fontWeight:900, color:"var(--text-color,#1e293b)", lineHeight:1, letterSpacing:"-1px" }}>{count}</div>
      <div style={{ fontSize:"0.73rem", color:"var(--text-muted,#6c757d)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", marginTop:5 }}>{label}</div>
    </div>
  );
};

const QuickBtn = ({ icon, label, onClick, color, badge }) => (
  <div onClick={onClick} style={{ background:"var(--card-bg,#fff)", borderRadius:14, border:"1.5px solid var(--border-color,#e8ecf0)", padding:"1.1rem 0.9rem", cursor:"pointer", textAlign:"center", position:"relative", transition:"all 0.22s" }}
    ...
    <div style={{ fontSize:"0.77rem", fontWeight:700, color:"var(--text-muted,#6c757d)", lineHeight:1.2 }}>{label}</div>
  </div>
);

const FeedCard = ({ title, icon, items, empty, renderRow, onAll, accent }) => (
  <div style={{ background:"var(--card-bg,#fff)", borderRadius:16, border:"1.5px solid var(--border-color,#e8ecf0)", overflow:"hidden", boxShadow:"0 4px 20px rgba(0,0,0,.05)" }}>
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"1.1rem 1.4rem", borderBottom:"1.5px solid var(--border-color,#e8ecf0)" }}>
      <span style={{ fontWeight:800, color:"var(--text-color,#1e293b)", display:"flex", alignItems:"center", gap:"0.5rem", fontSize:"0.95rem" }}>{icon} {title}</span>
...
export default function AdminDashboard({ showPage }) {
  ...
  return (
    <div style={{ background:"var(--bg-color,#f4f6fb)", minHeight:"100vh" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1e1b4b 0%,#312e81 35%,#4338ca 70%,#6366f1 100%)", padding:"2.5rem 0 4.5rem", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", bottom:0, left:0, right:0 }}>
          <svg viewBox="0 0 1440 56" fill="none" style={{ display:"block" }}><path d="M0,40 C360,0 1080,56 1440,16 L1440,56 L0,56 Z" fill="var(--bg-color,#f4f6fb)"/></svg>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"0 1.5rem 4rem" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"0.85rem" }}>
          <h3 style={{ fontWeight:800, color:"var(--text-color,#1e293b)", fontSize:"1rem", margin:0 }}>⚡ Quick Actions</h3>
        </div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"0.9rem" }}>
          <h3 style={{ fontWeight:800, color:"var(--text-color,#1e293b)", fontSize:"1rem", margin:0 }}>📡 Recent Activity</h3>
          <span style={{ fontSize:"0.78rem", color:"var(--text-muted,#94a3b8)", background:"var(--card-bg,#fff)", border:"1px solid var(--border-color,#e8ecf0)", borderRadius:8, padding:"0.25rem 0.7rem" }}>Live data</span>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:"1.4rem" }}>
            ...
            renderRow={item => {
              ...
              return <>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:"0.87rem", color:"var(--text-color,#1e293b)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{item.title}</div>
                  <div style={{ fontSize:"0.74rem", color:"var(--text-muted,#94a3b8)", marginTop:2 }}>📍 {item.location||"—"} · {fmtDate(item.createdAt)}</div>
                </div>
          ...
          <FeedCard title="Recent Claims" icon="📋" accent="#f59e0b"
            ...
            renderRow={claim => <>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:700, fontSize:"0.87rem", color:"var(--text-color,#1e293b)" }}>Claim #{claim._id?.slice(-5)}</div>
                <div style={{ fontSize:"0.74rem", color:"var(--text-muted,#94a3b8)", marginTop:2 }}>By {claim.claimantName} · {fmtDate(claim.createdAt)}</div>
              </div>
        ...
        {/* ── System Health ── */}
        <div style={{ marginTop:"2rem", background:"var(--card-bg,#fff)", borderRadius:16, border:"1.5px solid var(--border-color,#e8ecf0)", padding:"1.1rem 1.5rem", display:"flex", gap:"2rem", flexWrap:"wrap", alignItems:"center" }}>
          <div style={{ fontWeight:800, fontSize:"0.85rem", color:"var(--text-color,#1e293b)" }}>🔧 System Status</div>
          {["Database","API Server","Email Service","File Storage"].map(label => (
            <div key={label} style={{ display:"flex", alignItems:"center", gap:"0.4rem" }}>
              ...
              <span style={{ fontSize:"0.79rem", color:"var(--text-muted,#6c757d)", fontWeight:600 }}>{label}</span>
            </div>
          ))}
