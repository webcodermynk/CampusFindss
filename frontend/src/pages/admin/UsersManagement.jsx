...
const Modal = ({ open, onClose, title, children }) => open ? (
  <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
    <div onClick={e => e.stopPropagation()} style={{ background: "var(--card-bg,#fff)", borderRadius: 16, padding: "1.8rem", maxWidth: 480, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
...
export default function UsersManagement({ toast }) {
  ...
  if (loading) return <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}><div style={{ width: 44, height: 44, border: "4px solid var(--border-color,#dee2e6)", borderTopColor: "#3b82f6", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>;
  ...
  return (
    <div style={{ background: "var(--bg-color,#f8f9fa)", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: "0.8rem", marginBottom: "1.5rem" }}>
          {[
            ...
            <div key={label} style={{ background: "var(--card-bg,#fff)", borderRadius: 12, padding: "0.9rem 1rem", border: "1.5px solid var(--border-color,#dee2e6)", display: "flex", alignItems: "center", gap: "0.8rem" }}>
              ...
              <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-muted,#6c757d)", textTransform: "uppercase", letterSpacing: "0.04em" }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
          {["all","active","suspended"].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} style={{
              ...
              background: filterStatus === s ? "linear-gradient(135deg,#3b82f6,#6366f1)" : "var(--card-bg,#fff)",
              color: filterStatus === s ? "#fff" : "var(--text-muted,#6c757d)",
              borderColor: filterStatus === s ? "#3b82f6" : "var(--border-color,#dee2e6)",
            }}>{s}</button>
          ))}
          <span style={{ marginLeft: "auto", alignSelf: "center", color: "var(--text-muted,#6c757d)", fontSize: "0.85rem", fontWeight: 600 }}>{filtered.length} users</span>
        </div>
        ...
        <div style={{ background: "var(--card-bg,#fff)", borderRadius: 14, border: "1.5px solid var(--border-color,#dee2e6)", overflow: "hidden", boxShadow: "0 4px 15px rgba(0,0,0,0.05)" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.86rem" }}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={5} style={{ padding: "3rem", textAlign: "center", color: "var(--text-muted,#6c757d)" }}>No users found</td></tr>
                ) : filtered.map((u, i) => (
                  <tr key={u._id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border-color,#dee2e6)" : "none", transition: "background 0.15s" }}
                  ...
                  >
                    <td style={{ padding: "0.85rem 1rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
                        ...
                        <span style={{ fontWeight: 600, color: "var(--text-color,#2c3e50)" }}>{u.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: "0.85rem 1rem", color: "var(--text-muted,#6c757d)", fontSize: "0.84rem" }}>{u.email}</td>
                    <td style={{ padding: "0.85rem 1rem", color: "var(--text-muted,#6c757d)", fontFamily: "monospace", fontSize: "0.82rem" }}>{u.studentId || "—"}</td>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="⚠️ Delete User">
        <p style={{ color: "var(--text-muted,#6c757d)", marginBottom: "1.5rem" }}>
          Permanently delete <strong style={{ color: "var(--text-color,#2c3e50)" }}>{deleteTarget?.name}</strong>? All their data will be removed.
        </p>
        <div style={{ display: "flex", gap: "0.8rem", justifyContent: "flex-end" }}>
          <Btn onClick={() => setDeleteTarget(null)} color="var(--text-muted)" bg="var(--hover-bg,#f1f5f9)">Cancel</Btn>
